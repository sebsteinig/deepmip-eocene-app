...
with col1:
    projection = st.selectbox(
        label="projection", options=["Equirectangular", "Robinson", "Orthographic"], index=0, key="user_projection"
    )
with col2:
    outline_colour = st.selectbox(
        label="outline colour", options=["none", "gray", "red", "black", "white"], index=1, key="outline_colour"
    )
with col3:
    st.write("grid lines")
...
fig_models, progress_bar = plot_model_geographies(
    df_locations, projection, proxy_label, outline_colour, grid_check, labels_check
)
